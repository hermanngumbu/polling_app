...
import { PieChart, Share2 } from 'lucide-react';
import { QRCodeModal } from '@/components/QRCodeModal';
import { submitVote } from '@/lib/actions';

export function PollClientPage({ poll }) {
    ...
    <div className="max-w-2xl mx-auto py-8">
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Button variant="ghost" size="icon" onClick={() => setShowQRCode(true)}>
        ...
        <CardContent className="space-y-4">
          <form className="space-y-2">
            {poll.options.map((option) => (
              <Button
                key={option.id}
                formAction={() => submitVote(poll.id, option.id)}
      ...
      {showQRCode && <QRCodeModal url={pollUrl} onClose={() => setShowQRCode(false)} />}
    </div>
  );
}